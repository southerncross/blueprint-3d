...
# Usage: ./deploy.sh
#
# Note: $DEPLOY_USER_HOST should be set in environment already
# e.g. lishunyang@123.456.789

if [ ! $DEPLOY_USER_HOST ]; then
  echo '\033[31mERROR:\033[0m $DEPLOY_USER_HOST not found' && exit 1
fi
...
zip -r $zip_filename dist && scp $zip_filename $DEPLOY_USER_HOST:~/ && rm -Rf dist || exit 1

# Step 3: Call remote script
ssh -t $DEPLOY_USER_HOST "sh ./deploy_remote.sh ${filename}"
